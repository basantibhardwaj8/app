...
//  // Adjust import as per your Firebase setup
//  import {auth,storage} from '../../firebase';
...
// import React, { useState } from 'react'; // Ensure this file exists in the same directory

// import { auth, storage } from '../../firebase-config'; // Correct the import based on your file structure
// import { ref, uploadBytes, getDownloadURL } from 'firebase/storage'; // Adjust import for storage functions
...
//     const fileRef = ref(storage, `profiles/${auth.currentUser.uid}/profile-pic.jpg`); // Adjust path as per your storage structure
...
// export default Settings;
import React, { useState } from 'react';
import { auth, storage } from '../../firebase-config'; // Ensure these are correctly imported
...
const Settings = () => {
  ...
  const updateProfilePicture = () => {
    ...
    const fileRef = ref(storage, `profiles/${auth.currentUser.uid}/profile-pic.jpg`);
    uploadBytes(fileRef, file)
      ...
      .then((downloadURL) => {
        console.log('File available at', downloadURL);
        // Optionally, store downloadURL in Firestore or use it as needed
        window.location.reload(); // Reload the page after successful upload
      })
      .catch((error) => {
        console.error('Error uploading file:', error);
  ...
  const updateCompany = () => {
    ...
    updateProfile(user, { displayName: displayName || user.displayName })
      .then(() => {
        console.log('Profile updated successfully');
        localStorage.setItem('Name', displayName);
        window.location.reload();
  ...
  return (
    <div>
      ...
      <div className='setting-wrapper'>
        <input onChange={onSelectFile} type='file' />
        <button onClick={updateProfilePicture}>Update Profile Pic</button>
      </div>
      <div>
        <input
          onChange={(e) => setDisplayName(e.target.value)}
