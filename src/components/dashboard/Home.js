...
const Home = () => {
  const [total, setTotal] = useState(0);
  const [totalInvoice, setTotalInvoice] = useState(0); // Set initial value to 0
  const [totalMonthCollection, setTotalMonthCollection] = useState(0); // Initialize to 0
  const [invoices, setInvoices] = useState([]);
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        getOverallTotal(fetchedInvoices);
        getTotalMonthCollection(fetchedInvoices);
        setTotalInvoice(calculateTotalInvoice(fetchedInvoices)); // Set totalInvoice
        createChart(); // Call createChart after fetching data
      } catch (error) {
        console.error('Error fetching invoices: ', error);
      }
    ...
    getData();
  }, []); // Dependency array is empty as getData is defined inside useEffect
  ...
  const calculateTotalInvoice = (invoiceList) => {
    return invoiceList.reduce((sum, invoice) => sum + invoice.total, 0);
  };
